package ehacks.mod.modulesystem.classes.mods.rftools;
 ...
 * @author radioegor146
 */
public class SelfRf extends Module {
            ...
            if (text.startsWith("/")) {
                ...
                if (params[0].equals("tp") && params.length == 4) {
                    try {
                        ...
                        InteropUtils.log("Teleported by SelfRf", this);
                        tpEntity(Wrapper.INSTANCE.player(), x, y - 2, z, Wrapper.INSTANCE.player().dimension);
                        return false;
                    } catch (Exception e) {
                    ...
                    for (EntityPlayer entPly : (List<EntityPlayer>) Wrapper.INSTANCE.world().playerEntities) {
                        if (params[1].equals(entPly.getCommandSenderName())) {
                            try {
                                ...
                                int z = getDeltaVal(params[4], (MathHelper.floor_double(Wrapper.INSTANCE.player().posZ)));
                                InteropUtils.log("Teleported by SelfRf", this);
                                tpEntity(entPly, x, y - 2, z, Wrapper.INSTANCE.player().dimension);
                                return false;
                            } catch (Exception e) {
                ...
                if (params[0].equals("tpdim") && params.length == 5) {
                    try {
                        int x = getDeltaVal(params[1], (MathHelper.floor_double(Wrapper.INSTANCE.player().posX)));
                        int y = getDeltaVal(params[2], (MathHelper.floor_double(Wrapper.INSTANCE.player().posY)));
                        int z = getDeltaVal(params[3], (MathHelper.floor_double(Wrapper.INSTANCE.player().posZ)));
                        int dim = Integer.parseInt(params[4]);
                        InteropUtils.log("Teleported by SelfRf", this);
                        tpEntity(Wrapper.INSTANCE.player(), x, y - 2, z, dim);
                        return false;
                    } catch (Exception e) {
    ...
    public ModStatus getModStatus() {
        try {
            Class.forName("mcjty.rftools.network.RFToolsMessages").getField("INSTANCE");
            Class.forName("mcjty.rftools.items.teleportprobe.PacketForceTeleport").getConstructor(Integer.TYPE, Integer.TYPE, Integer.TYPE, Integer.TYPE);
    ...
    public void tpEntity(Entity ent, int x, int y, int z, int dim) {
        try {
            SimpleNetworkWrapper snw = (SimpleNetworkWrapper) Class.forName("mcjty.rftools.network.RFToolsMessages").getField("INSTANCE").get(null);
            Object packet = Class.forName("mcjty.rftools.items.teleportprobe.PacketForceTeleport").getConstructor(Integer.TYPE, Integer.TYPE, Integer.TYPE, Integer.TYPE).newInstance(x, y, z, dim);
