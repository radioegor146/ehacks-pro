...
public class NCPSpeed
    ...
    public void onTicks() {
        ...
        double slow = 1.458;
        double offset = 4.7;
        float timer = this.timerSpeed;
        boolean shouldOffset = true;
        this.canStep = false;
        for (Object o : Wrapper.INSTANCE.world().getCollidingBoundingBoxes(Wrapper.INSTANCE.player(), Wrapper.INSTANCE.player().boundingBox.copy().offset(Wrapper.INSTANCE.player().motionX / offset, 0.0, Wrapper.INSTANCE.player().motionZ / offset))) {
