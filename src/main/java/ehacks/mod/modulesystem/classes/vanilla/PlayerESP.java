...
public class PlayerESP
    ...
    public String getDescription() {
        return "Allows you to see all of the players around you";
    }
    ...
    public void onWorldRender(RenderWorldLastEvent event) {
        ...
        entities.stream().filter((ent) -> !(!(ent instanceof EntityPlayer))).filter((ent) -> !(ent == Wrapper.INSTANCE.player())).filter((ent) -> !(!ent.isInRangeToRender3d(Wrapper.INSTANCE.mc().renderViewEntity.getPosition(event.partialTicks).xCoord, Wrapper.INSTANCE.mc().renderViewEntity.getPosition(event.partialTicks).yCoord, Wrapper.INSTANCE.mc().renderViewEntity.getPosition(event.partialTicks).zCoord) || ent == Wrapper.INSTANCE.mc().renderViewEntity && Wrapper.INSTANCE.mcSettings().thirdPersonView == 0 && !Wrapper.INSTANCE.mc().renderViewEntity.isPlayerSleeping())).forEachOrdered((ent) -> {
        ...
        Wrapper.INSTANCE.world().loadedEntityList.stream().map((o) -> (Entity) o).filter((entObj) -> !(!(entObj instanceof EntityPlayer))).filter((entObj) -> !(entObj == Wrapper.INSTANCE.player())).map((entObj) -> (EntityLivingBase) entObj).map((ent) -> {
            ...
            EntityLivingBase ent = (EntityLivingBase)ent1;
