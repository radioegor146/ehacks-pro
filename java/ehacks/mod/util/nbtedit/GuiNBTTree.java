...
public class GuiNBTTree
            ...
            if (!reInit) {
                ...
                for (GuiSaveSlotButton button : saves) {
                    if (button.inBoundsOfX(mx, my)) {
                        ...
                        Wrapper.INSTANCE.mc().getSoundHandler().playSound((ISound) PositionedSoundRecord.func_147674_a((ResourceLocation) new ResourceLocation("gui.button.press"), (float) 1.0f));
                        return;
                    }
                    if (!button.inBounds(mx, my)) {
                        continue;
    ...
    private void saveButtonClicked(GuiSaveSlotButton button) {
        if (saveSlots[button.saveId] == null) {
            ...
            Wrapper.INSTANCE.mc().getSoundHandler().playSound((ISound) PositionedSoundRecord.func_147674_a((ResourceLocation) new ResourceLocation("gui.button.press"), (float) 1.0f));
            Wrapper.INSTANCE.mc().displayGuiScreen((GuiScreen) new GuiNBTEdit(saveSlots[button.saveId]));
        }
    }
