...
public class WindowInfo
    ...
    public void draw(int x, int y) {
        if (this.isOpen()) {
            if (this.isExtended()) {
                this.setClientSize(88, 46);
                super.draw(x, y);
                Wrapper.INSTANCE.fontRenderer().drawStringWithShadow(Wrapper.INSTANCE.mc().debug.split(",")[0].toUpperCase(), this.getClientX() + 1, this.getClientY() + 1, 5636095);
