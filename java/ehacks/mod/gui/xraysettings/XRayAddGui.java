...
extends GuiScreen {
    ...
    protected void actionPerformed(GuiButton par1GuiButton) {
        if (par1GuiButton.id == 0) {
            ...
            this.mc.displayGuiScreen((GuiScreen)new XRayGui());
    ...
    protected void keyTyped(char par1, int par2) {
        ...
        for (Object string : s) {
            ...
            if (!b2.getLocalizedName().toLowerCase().contains(sb.toLowerCase())) continue;
    ...
    private stringint getClickedBlock(int x, int y) {
        ...
        for (i = 0; i < this.blocks.size(); ++i) {
            ...
            for (int j = 0; j < blockstodraw.size(); ++j) {
                int var14;
                if ((index + j) % 9 > 9 || (index + j - this.sliderpos * 9) / 9 > 7 || index + j - this.sliderpos * 9 < 0) continue;
                int c = (index + j) % 9;
                int v = (index + j - this.sliderpos * 9) / 9;
                if (x <= 10 + c * 20 || x >= 26 + c * 20 || y <= v * 20 + 60 || y >= v * 20 + 76) continue;
                boolean smeta = false;
                try {
