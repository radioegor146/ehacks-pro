...
extends Mod {
    ...
    public void onWorldRender(RenderWorldLastEvent event) {
        ...
        while (!hasLanded) {
            ...
            for (int index = 0; index < entities.size(); ++index) {
                ...
                if (!entity.canBeCollidedWith() || entity == player || (possibleEntityLanding = (var12 = entity.boundingBox.expand((double)(var11 = 0.3f), (double)var11, (double)var11)).calculateIntercept(present, future)) == null) continue;
                hasLanded = true;
                isEntity = true;
        ...
        for (int x = chunkMinX; x < chunkMaxX; ++x) {
            for (int y = chunkMinY; y < chunkMaxY; ++y) {
                for (int z = chunkMinZ; z < chunkMaxZ; ++z) {
                    ...
                    if (block == null || block.getMaterial() != material || (double)chunkMaxY < (liquidHeight = (double)((float)(y + 1) - BlockLiquid.getLiquidHeightPercent((int)Wrapper.INSTANCE.world().getBlockMetadata(x, y, z))))) continue;
                    isWithin = true;
                }
            }
        }
    ...
    private List getEntitiesWithinAABB(AxisAlignedBB axisalignedBB) {
        ...
        for (int x = chunkMinX; x <= chunkMaxX; ++x) {
            for (int z = chunkMinZ; z <= chunkMaxZ; ++z) {
                if (!Wrapper.INSTANCE.world().getChunkProvider().chunkExists(x, z)) continue;
