...
public class SelfEnd extends Module {
    ...
    public void onWorldRender(RenderWorldLastEvent event) {
        ...
        double xPos = (Wrapper.INSTANCE.mc().renderViewEntity.lastTickPosX + distVec.xCoord) + (Wrapper.INSTANCE.mc().renderViewEntity.posX - Wrapper.INSTANCE.mc().renderViewEntity.lastTickPosX) * (double) event.partialTicks;
        double yPos = (Wrapper.INSTANCE.mc().renderViewEntity.lastTickPosY + distVec.yCoord + 1) + (Wrapper.INSTANCE.mc().renderViewEntity.posY - Wrapper.INSTANCE.mc().renderViewEntity.lastTickPosY) * (double) event.partialTicks;
        double zPos = (Wrapper.INSTANCE.mc().renderViewEntity.lastTickPosZ + distVec.zCoord) + (Wrapper.INSTANCE.mc().renderViewEntity.posZ - Wrapper.INSTANCE.mc().renderViewEntity.lastTickPosZ) * (double) event.partialTicks;
        ...
        if (packet instanceof C01PacketChatMessage) {
            ...
            if (text.startsWith("/")) {
                ...
                if (params[0].equals("tp") && params.length == 4) {
                    try {
                        int x = getDeltaVal(params[1], (int) (MathHelper.floor_double(Wrapper.INSTANCE.player().posX)));
                        int y = getDeltaVal(params[2], (int) (MathHelper.floor_double(Wrapper.INSTANCE.player().posY)));
                        int z = getDeltaVal(params[3], (int) (MathHelper.floor_double(Wrapper.INSTANCE.player().posZ)));
                ...
                if (params[0].equals("tp") && params.length == 5) {
                    if (params[1].equals(Wrapper.INSTANCE.player().getCommandSenderName())) {
                        try {
                            int x = getDeltaVal(params[2], (int) (MathHelper.floor_double(Wrapper.INSTANCE.player().posX)));
                            int y = getDeltaVal(params[3], (int) (MathHelper.floor_double(Wrapper.INSTANCE.player().posY)));
                            int z = getDeltaVal(params[4], (int) (MathHelper.floor_double(Wrapper.INSTANCE.player().posZ)));
                            InteropUtils.log("Teleported by SelfEnd", this);
                            tpEntity(Wrapper.INSTANCE.player(), x, y - 2, z);
                            return false;
                        } catch (Exception e) {
