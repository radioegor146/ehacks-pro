...
extends Module {
    ...
    public boolean onPacket(Object packet, Side side) {
        if (fakeGuiOpened && packet instanceof net.minecraft.network.play.server.S2FPacketSetSlot) {
            net.minecraft.network.play.server.S2FPacketSetSlot packet2 = (net.minecraft.network.play.server.S2FPacketSetSlot)packet;
            if (packet2.func_149173_d() == -1) {
                return false;
            }
            int slotId = 0;
            if (packet2.func_149173_d() < 9)
                slotId = packet2.func_149173_d() + 36 - 5;
            else
            {
                if (packet2.func_149173_d() > 35)
                    slotId = packet2.func_149173_d() - 36;
                else
                    slotId = packet2.func_149173_d();
            }
            Wrapper.INSTANCE.player().inventory.setInventorySlotContents(slotId, packet2.func_149174_e());
            return false;
        }
        return true;
    }
